...
const features = [
  {
    ...
    description: "Track all bookings and revenue streams in real-time with customizable dashboards and reports.",
    icon: BarChart3,
    color: "from-blue-500 to-cyan-500",
  },
  {
    title: "Live Vehicle Status & Fuel Tracking",
    description: "Monitor your entire fleet with GPS tracking, maintenance alerts, and fuel consumption analytics.",
    icon: Truck,
...
const AnimatedText = ({ text, className, id }: { text: string; className?: string; id?: string }) => {
  const words = text.split(" ")

  return (
    <motion.div variants={textVariants} initial="hidden" animate="visible" className={className} id={id}>
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            Answer a few questions and get a personalized solution for your fleet operations.
          </motion.p>
              ...
              <Card className="h-full hover:shadow-2xl transition-all duration-500 border-0 shadow-md bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm group overflow-hidden relative">
                ...
                <CardContent className="p-8 relative z-10">
                  <motion.div
                    ...
                    transition={{ duration: 0.3 }}
                  >
                    <div className={`p-3 bg-gradient-to-br ${feature.color} rounded-lg mr-4 shadow-lg`}>
                      <feature.icon className="h-6 w-6 text-white" />
                    </div>
                  </motion.div>
                  <h3
                    className="text-xl font-semibold text-slate-800 dark:text-slate-100 mb-3 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-300"
                    id={`feature-${index}`}
                  >
                    {feature.title}
                  </h3>
